...
class NewCandidate extends Component {
    ...
    addCandidates() {
        ...
        this.state.election.methods
            ...
            .then(async (receipt) => {
                ...
                try {
                    await axios.post('http://localhost:8000/api/candidate', {
                        election_id: election_id,
                        name: this.state.candidate_name,
                        details: this.state.candidate_details
                    });
                    ...
                    alert("Candidate added to blockchain but failed to save to database");
                    this.setState({ loading: false });
                }
            })
            .catch((error) => {
    ...
    async fetchCandidates() {
        try {
            ...
            // Set candidates directly from database
            this.setState({ candidates: dbCandidates });
            
        } catch (err) {
            console.error('Failed to fetch candidates:', err);
            this.setState({ error: 'Failed to fetch candidates.' });
        }
    }

    handleRemoveCandidate = async (candidateId) => {
        ...
        try {
            ...
            await axios.delete(`http://localhost:8000/api/candidate/${candidateId}`);
            ...
            this.setState({ 
                error: 'Failed to remove candidate. Please try again.',
                loading: false 
            });
        }
    }
    ...
    render() {
        const { loading, error, isAdmin, candidates } = this.state;
        ...
        return(
            <div className="container">
                ...
                <h5 style={{marginTop: '40px'}}>Candidates</h5>
                <div className="collection">
                    {candidates.map(candidate => (
                        <div key={candidate.candidate_id} className="collection-item">
                            <div className="row" style={{marginBottom: 0}}>
                                <div className="col s12 m8">
                                    <span><b>{candidate.name}</b>: {candidate.details}</span>
                                </div>
                                {isAdmin && (
                                    <div className="col s12 m4 right-align">
                                        <button 
                                            className="btn red" 
                                            onClick={() => this.handleRemoveCandidate(candidate.candidate_id)}
                                            disabled={loading}
                                        >
                                            Remove
                                        </button>
